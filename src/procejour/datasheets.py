...
from procejour.models import Datasheet, User
...
async def run_datasheet(datasheet_id: int, current_user: CurrentUser):
    ...
    with ui.list().classes("w-full"):
        for step in procedure_rev.steps:
            await build_procedure_step(datasheet, step, current_user)


async def build_procedure_step(
    ...
    if step.get("heading", False):
        ...
        await pass_fail_action_step(step, datasheet, current_user)
    else:
        await simple_action_step(step, datasheet, current_user)
